...
const globalStyle = css`*, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; } body { background: ${C.bg}; color: ${C.text}; font-family: 'Rajdhani', sans-serif; -webkit-font-smoothing: antialiased; } ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: ${C.panel}; } ::-webkit-scrollbar-thumb { background: ${C.border}; border-radius: 2px; } select, input, option { font-family: inherit; } @keyframes fadeIn { from { opacity:0; transform:translateY(6px); } to { opacity:1; transform:translateY(0); } } @keyframes slideUp { from { opacity:0; transform:translateY(40px); } to { opacity:1; transform:translateY(0); } }`;
...
.filter(g => !search || byGarage(g).length > 0);

// 重複車輛：依車名分組，只保留出現 2 次以上的，依車名排序
const duplicates = (() => {
const map = {};
vehicles.forEach(v => {
const key = v.車名.trim();
...
```
  ...
  <div style={{ padding:"10px 16px", display:"flex", gap:8, borderBottom:`1px solid ${C.border}`, background:C.panel }}>
    ...
    <button style={{ ...btnG, padding:"8px 10px", fontSize:11, whiteSpace:"nowrap", color: duplicates.length > 0 ? C.red : C.muted, borderColor: duplicates.length > 0 ? `${C.red}66` : C.border }} onClick={() => setModal("dupes")}>
        ...
        <div key={garage} style={{ marginBottom:28, animation:"fadeIn .3s ease" }}>
          ...
          {cars.length === 0 ? (
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              ...
              {/* 空車位 */}
              {Array.from({ length: Math.max(0, cap - used) }).map((_, i) => (
                <div key={`empty-${i}`} style={{ border:`1px dashed ${C.border}`, borderRadius:6, padding:"10px 12px", display:"flex", alignItems:"center", gap:10, opacity:0.4 }}>
                  <div style={{ color:C.muted, fontSize:11, width:20, textAlign:"center", flexShrink:0 }}>{used + i + 1}</div>
                  <div style={{ fontSize:12, color:C.muted, letterSpacing:1 }}>空車位</div>
                </div>
              ))}
            </div>
          )}
  ...
  {/* Duplicates Modal */}
  {modal === "dupes" && (
    <Modal title={`重複車輛 (${duplicates.length})`} onClose={() => setModal(null)}>
      {duplicates.length === 0 ? (
        <div style={{ color:C.muted, textAlign:"center", padding:"20px 0", letterSpacing:1 }}>沒有重複車輛 ✓</div>
      ) : (
          ...
          {duplicates.map(([name, arr]) => (
            <div key={name}>
              <div style={{ fontSize:13, fontWeight:700, color:C.accent, marginBottom:6, display:"flex", alignItems:"center", gap:8 }}>
                {name}
                <span style={{ background:`${C.red}22`, color:C.red, border:`1px solid ${C.red}44`, borderRadius:10, padding:"1px 8px", fontSize:10, fontWeight:900 }}>×{arr.length}</span>
              </div>
              {arr.map(v => (
                <div key={v.id} style={{ background:"#16161f", border:`1px solid ${C.border}`, borderRadius:4, padding:"8px 10px", marginBottom:4, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    <span style={{ fontSize:12, color:C.text }}>{v.車庫}</span>
                    {v.備註 && <span style={{ fontSize:11, color:C.muted, marginLeft:8 }}>{v.備註}</span>}
                  </div>
                  <div style={{ display:"flex", gap:5 }}>
                    <button style={{ ...btnG, padding:"4px 8px", fontSize:10 }} onClick={() => { setSelected(v); setMoveTarget(""); setModal("move"); }}>移動</button>
                    <button style={{ ...btnG, padding:"4px 8px", fontSize:10, color:C.red, borderColor:`${C.red}44` }} onClick={() => deleteVehicle(v.id)}>刪除</button>
                  </div>
                </div>
              ))}
            </div>
